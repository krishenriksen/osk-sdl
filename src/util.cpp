...
int handleTapBegin(unsigned xTapped, unsigned yTapped, int screenHeight, Keyboard &kbd)
{
	...
	if (!key.keyChar.empty()) {
		// space key is longer
		if (key.keyChar == KEYCAP_SPACE) {
			return 235;
		}
		else if (key.keyChar == KEYCAP_RETURN) {
			return 47;
		}
		else if (key.keyChar == KEYCAP_BACKSPACE) {
			return 47;
		}
